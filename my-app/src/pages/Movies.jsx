...
function Movies() {
  ...
  const shown = movies.filter(m => m.title.toLowerCase().includes(Query.toLowerCase()) && (selectedGenre === '' || m.genre === selectedGenre));


  return (
    <div className="mx-auto max-w-5xl p-8">
      <h1 className="mb-6 text-2xl font-bold text-slate-800">หนังทั้งหมด</h1>
      <GenreBox genre={selectedGenre} onGenreChange={setSelectedGenre} genres={[...new Set(movies.map(m => m.genre))]} />
